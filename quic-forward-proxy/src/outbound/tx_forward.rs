...
pub async fn tx_forwarder(
    ...
    loop {
        ...
        let forward_packet = Arc::new(transaction_channel
            .recv()
            .await
            .expect("channel closed unexpectedly"));
            ...
            for connection_idx in 1..PARALLEL_TPU_CONNECTION_COUNT {
                ...
                tokio::spawn(async move {
                    ...
                    while let Ok(packet) = per_connection_receiver.recv().await {
                        ...
                        let mut transactions_batch: Vec<VersionedTransaction> = packet.transactions.clone();
        ...
        broadcast_in.send(forward_packet)
